...
function extractPlanPaths(mdText, phaseFilter /* number|null */) {
  const roots = [
    ...
    "tools/",
    ".plan_guard/",
    // Optional safe roots to keep
    "scripts/",
    "public/",
    // Root docs (allowed tokens)
    "index.html",
    "netlify.toml",
    "package.json",
    "README.md",
  ...
  // 1) backticked paths
  {
    const re = /`([^`\n\r]+)`/g;
    let m;
    const scopedNoCode = scoped.replace(/\x60/g, "");
    while ((m = re.exec(scopedNoCode)) !== null) {
  ...
  {
    const re =
      /\b(assets\/[A-Za-z0-9._\-\/]+|pages\/[A-Za-z0-9._\-\/]+|data\/[A-Za-z0-9._\-\/]+|docs\/[A-Za-z0-9._\-\/]+|dives\/[A-Za-z0-9._\-\/]+|netlify\/[A-Za-z0-9._\-\/]+|tools\/[A-Za-z0-9._\-\/]+|\.plan_guard\/[A-Za-z0-9._\-\/]+|scripts\/[A-Za-z0-9._\-\/]+|public\/[A-Za-z0-9._\-\/]+|index\.html|netlify\.toml|package\.json|README\.md|master_build\.md|MASTER_BUILD_DIRECTIONS\.md|PHASE_LOG\.md|PROTOCOLS\.md|PHASE_1_FILELIST\.md|PHASE_2_FILELIST\.md)\b/g;
    let m;
    const scopedNoCode = scoped.replace(/\x60/g, "");
    while ((m = re.exec(scopedNoCode)) !== null) {
      const candidate = accept(m[1]);
